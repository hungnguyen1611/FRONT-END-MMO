...
const fetchStoreService = async (id) => {
...
const StoreService = () => {
  ...
  const { data, isLoading, error } = useQuery(['storeService', id], () => fetchStoreService(id), {
    onSuccess: (data) => {
      // setFilteredArr(data)
      ...
      <div className='w-100 d-flex'>
        ...
        <div className='w-100'>
          <div className='border mt-5 mt-xl-0 p-2 w-100'>
            <h3 className='mb-3'>Đơn hàng dịch vụ</h3>
                  ...
                  {Array.isArray(filteredArr) && filteredArr?.length > 0 ? (
                    currentItems.map((a, i) => (
                      <tr key={i} className='mb-2'>
                        ...
                        <td>{a?.finishDay}</td>
                        <td className='service-booth'>{a?.detail?.id_detailBooth?.name}</td>
                        <td>{a?.id_user?.username}</td>
                        <td className='service-booth'>{a?.detail?.id_boothService?.nameBooth}</td>
                        ...
                        <td className='d-flex gap-3'>
                          <img
                            ...
                            alt=''
                          />
                          {/* <img style={{ maxWidth: '20px', cursor: 'pointer' }} src={remove} alt='' /> */}
                        </td>
                      </tr>
                    ))
                  ) : (
